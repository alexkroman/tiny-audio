...
struct VADTests {
    @Test func sileroDetectsSilenceVsSpeechLikeSignal() throws {
        ...
        #expect(maxProb > probSilence, "speech-like signal should beat silence prob (got max=\(maxProb))")
    }
    ...
    @Test func streamerEmitsOnsetThenOffsetOnSilencePattern() throws {
        ...
        for _ in 0 ..< 5 {
            allEvents.append(contentsOf: try streamer.process([Float](repeating: 0, count: SileroVAD.frameSize)))
        }
        for _ in 0 ..< 15 {
        ...
        let hasOnset = allEvents.contains { if case .onset = $0 { return true }; return false }
        let hasOffset = allEvents.contains { if case .offset = $0 { return true }; return false }
