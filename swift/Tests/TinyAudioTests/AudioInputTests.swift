...
struct AudioInputTests {
    ...
    @Test func allInputsConvergeToSameSamples() throws {
        ...
        let fromSamples = try AudioDecoder.decode(.samples(nativeSamples, sampleRate: buffer.format.sampleRate))

        #expect(fromFile.count == fromBuffer.count, "file and buffer paths produce different lengths")
        #expect(fromBuffer.count == fromSamples.count, "buffer and samples paths produce different lengths")
