...
struct ManifestVerifier {
    ...
    static func verify(directory: URL) throws {
        ...
        for (name, entry) in manifest.files {
            ...
            let actualSize = (try? FileManager.default.attributesOfItem(atPath: fileURL.path)[.size] as? Int) ?? -1
            guard actualSize == entry.size else {
                throw TinyAudioError.manifestMismatch(file: name, expected: "size=\(entry.size)", actual: "size=\(actualSize)")
            }
