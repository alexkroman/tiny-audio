...
final class VADStreamer {
    ...
    /// Feed a 512-sample frame at 16 kHz. Returns 0 or 1 events.
    func process(_ frame: [Float]) throws -> [Event] {
        ...
        switch state {
        case .idle:
            // Track pre-speech audio in a ring buffer.
            for s in frame {
                if !preSpeechRing.isEmpty {
                    preSpeechRing[preSpeechRingHead] = s
                    preSpeechRingHead = (preSpeechRingHead + 1) % preSpeechRing.count
                }
            }
            if isSpeech {
