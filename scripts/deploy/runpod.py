...
# =============================================================================
# Add NeuCodec Codes to SIFT Dataset Command
# =============================================================================


def build_add_codes_script(
    hf_token: str,
    input_repo: str,
    output_repo: str,
    ...
    """Generate the add-codes script content."""
    max_samples_arg = f"--max-samples {max_samples}" if max_samples else ""
...
python -m scripts.generate_sift_dataset add-codes \\
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "===== Add Codes Completed Successfully ====="
else
    echo "===== Add Codes Failed with exit code: $EXIT_CODE ====="
fi
...
def add_codes(
    host: str = typer.Argument(..., help="RunPod instance IP address or hostname"),
    port: int = typer.Argument(..., help="SSH port for the RunPod instance"),
    input_repo: str = typer.Option(
        "mazesmazes/sift-audio-2", "--input-repo", "-i", help="Input SIFT dataset repo"
    ),
    output_repo: str = typer.Option(
        "mazesmazes/sift-audio-2", "--output-repo", "-o", help="Output repo (can be same as input)"
    ),
    splits: Annotated[
        list[str] | None,
        typer.Option("--splits", "-s", help="Specific splits to process (default: all)"),
    ] = None,
    session_name: str | None = typer.Option(
        None, "--session-name", help="Custom tmux session name"
    ),
    batch_size: int = typer.Option(16, "--batch-size", "-b", help="Batch size for TTS + NeuCodec"),
    max_samples: int | None = typer.Option(
    ...
    """Add NeuCodec codes to an existing SIFT dataset using Kokoro TTS.

    This generates response_codes for each sift_response text, enabling
    joint training of projector + audio head.

    Examples:
        ta runpod add-codes host port --splits podcast
        ta runpod add-codes host port --input-repo user/sift --output-repo user/sift-codes
    """
    conn = get_connection(host, port)
    ...
    if session_name is None:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M")
        session_name = f"addcodes_{timestamp}"
    ...
    # Build and upload script
    script_content = build_add_codes_script(
        hf_token, input_repo, output_repo, splits, batch_size, max_samples
    )
    script_path = f"/tmp/addcodes_{session_name}.sh"

    print(f"\nStarting add-codes session '{session_name}'...")
    print(f"Input repo: {input_repo}")
    print(f"Output repo: {output_repo}")
    print(f"Batch size: {batch_size}")
    if splits:
    ...
    print(f"\nAdd-codes generation started in session '{session_name}'.")
    print(f"To re-attach later: ssh -p {port} root@{host} -t 'tmux attach -t {session_name}'")
...
# =============================================================================
# NeuCodec Dataset Generation Command
# =============================================================================
...
def build_neucodec_script(
    ...
    """Generate the NeuCodec dataset generation script content."""
    max_samples_arg = f"--max-samples {max_samples}" if max_samples else ""
...
python -m scripts.generate_neucodec_dataset \\
    --output-repo {output_repo} \\
    --split {split} \\
    --batch-size {batch_size} \\
    {max_samples_arg}
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "===== NeuCodec Dataset Generation Completed Successfully ====="
else
    echo "===== NeuCodec Dataset Generation Failed with exit code: $EXIT_CODE ====="
fi
...
def neucodec(
    ...
    port: int = typer.Argument(..., help="SSH port for the RunPod instance"),
    output_repo: str = typer.Option(
        "mazesmazes/libritts-r-neucodec", "--output-repo", "-o", help="HuggingFace repo for output"
    ),
    split: str = typer.Option("train.clean.360", "--split", "-s", help="Dataset split to process"),
    session_name: str | None = typer.Option(
        None, "--session-name", help="Custom tmux session name"
    ),
    batch_size: int = typer.Option(1024, "--batch-size", "-b", help="Batch size for encoding"),
    max_samples: int | None = typer.Option(
        None, "--max-samples", "-n", help="Max samples to process"
    ),
    no_attach: bool = typer.Option(False, "--no-attach", help="Start session but don't attach"),
    force: bool = typer.Option(False, "--force", "-f", help="Kill existing session with same name"),
):
    """Generate NeuCodec-encoded dataset on a remote RunPod instance.

    Pre-computes NeuCodec tokens for LibriTTS-R, enabling fast S2S training
    without on-the-fly encoding.

    Example:
        ta runpod neucodec host port --split train.clean.100
        ta runpod neucodec host port --split train.clean.360 --output-repo user/dataset
    """
    conn = get_connection(host, port)
    ...
    # Build and upload script
    script_content = build_neucodec_script(hf_token, output_repo, split, batch_size, max_samples)
    script_path = f"/tmp/neucodec_{session_name}.sh"

    print(f"\nStarting NeuCodec generation session '{session_name}'...")
    print(f"Output repo: {output_repo}")
    print(f"Split: {split}")
    print(f"Batch size: {batch_size}")
    if max_samples:
    ...
    print(f"\nNeuCodec generation started in session '{session_name}'.")
    print(f"To re-attach later: ssh -p {port} root@{host} -t 'tmux attach -t {session_name}'")
