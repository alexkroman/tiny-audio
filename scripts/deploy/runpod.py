    ...
    if not session_name:
        ...
        if len(sessions) == 1:
            ...
            for i, session in enumerate(sessions, 1):
                print(f"  {i}. {session}")
            # typer.prompt re-asks on non-integer input and turns Ctrl+C into a
            # clean "Aborted" exit; only the range check is ours.
            choice = typer.prompt(f"Enter number (1-{len(sessions)})", type=int)
            if not 1 <= choice <= len(sessions):
                raise typer.BadParameter(f"choose a number between 1 and {len(sessions)}")
