...
class TestDiarizationDatasets:
    ...
    def test_diarization_configs_have_speaker_fields(self):
        ...
        for name in DIARIZATION_DATASETS:
            ...
            err = f"Missing timestamps_start_field for {name}"
            assert config.timestamps_start_field is not None, err
            err = f"Missing timestamps_end_field for {name}"
            assert config.timestamps_end_field is not None, err
